...
# Modified to allow more parameters than just Z
...
class TorchMD_Net(nn.Module):
    ...
    def forward(
        ...
        if torch.isnan(pos).any():
            warnings.warn("NaN in pos")
            y = torch.inf*pos.sum()[None, None]

        else:
            # run the potentially wrapped representation model
            x, v, z, _, batch = self.representation_model(
                data, pos, batch, box=box, q=q, s=s
            )
            ...
            if self.prior_model is not None:
                for prior in self.prior_model:
                    x = prior.pre_reduce(x, data, pos, batch, extra_args)
            ...
            if self.prior_model is not None:
                for prior in self.prior_model:
                    y = prior.post_reduce(y, data, pos, batch, box, extra_args)
