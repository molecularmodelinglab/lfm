...
class TensorNet(nn.Module):
    ...
    def forward(
        ...
        if q is None:
            q = torch.zeros((z.shape[0],), device=z.device, dtype=z.dtype)
        else:
            q = q[batch]
        zp = z
        if self.static_shapes:
            mask = (edge_index[0] < 0).unsqueeze(0).expand_as(edge_index)
            zp = torch.cat((z, torch.zeros((1, z.shape[-1]), device=z.device, dtype=z.dtype)), dim=0)
...
class TensorEmbedding(nn.Module):
    ...
    def __init__(
        ...
        self.max_z = max_z
        self.emb = nn.Linear(hidden_channels, hidden_channels, dtype=dtype)
