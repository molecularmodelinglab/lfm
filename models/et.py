...
# This is modified from the original; I just made it take as input an abitrary input
# rather than just Z and pos
...
class TorchMD_ET(nn.Module):
    ...
    def forward(
        ...
        edge_atoms: Optional[Tensor] = None,
    ) -> Tuple[Tensor, Tensor, Tensor, Tensor, Tensor]:
        """ If edge_atoms is not None, only the interactions between the atoms in edge_atoms 
        (or between edge atoms and other atoms) are considered. """
        ...
        if edge_atoms is not None:
            ...
            idx1 = edge_index[0]
            idx2 = edge_index[1]
            edge_mask = (torch.isin(idx1, edge_atoms) | torch.isin(idx2, edge_atoms))
...
class EquivariantMultiHeadAttention(nn.Module):
    ...
    def forward(self, x, vec, edge_index, r_ij, f_ij, d_ij):
        ...
        x, vec = self.propagate(
            ...
            r_ij=r_ij,
            d_ij=d_ij,
            dim_size=x.size(0), # maybe commit to torchmd-net?
        )
