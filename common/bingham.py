...
This was taken from https://github.com/igilitschenski/deep_bingham/blob/master/bingham_distribution.py
and lightly adapted
...
class BinghamDistribution(object):
    ...
    def logpdf_unnorm(self, data):
        """ (unnormalized) log PDF of the Bingham Distribution
        ...
        for i in range(0, num_data_points):
            density[i] = np.dot(data[i], np.dot(full_param_matrix, data[i]))
        density = density
        return density

    def logpdf(self, data):
        return self.logpdf_unnorm(data) - np.log(self._norm_const)

    def pdf(self, data):
        return np.exp(self.logpdf(data))

    def random_samples_old(self, n):
    ...
    def random_samples(self, n):
        """
        A vectorized version of the above function; about 300x faster
        """
