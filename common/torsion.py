...
class TorsionData(Batchable):
    ...
    @staticmethod
    def from_pyg(data):
        return TorsionData.from_nx_graph(to_networkx(data, edge_attrs=["edata"]))

    def to(self, device):
        return TorsionData(self.rot_edges.to(device), self.rot_masks.to(device))
    ...
    def from_nx_graph(G):
        ...
        to_rotate = []
        for edge in G.edges:
            bond_order = G.get_edge_data(*edge)["edata"]
            G2 = G.to_undirected()
            G2.remove_edge(*edge)
            if not nx.is_connected(G2) and bond_order == 1.0:
